...
const getAuthErrorMessage = (code: string) => {
  switch (code) {
    ...
    case 'auth/user-not-found': return "لا يوجد حساب مسجل بهذا البريد الإلكتروني. يرجى التأكد من كتابته بشكل صحيح أو إنشاء حساب جديد.";
    case 'auth/wrong-password': return "كلمة المرور غير صحيحة.";
    case 'auth/invalid-credential': return "بيانات الدخول غير صحيحة أو لا يوجد حساب مسجل بهذا البريد الإلكتروني.";
    case 'auth/missing-email': return "يرجى كتابة البريد الإلكتروني أولاً.";
...
export default function LoginPage() {
  ...
  const handleResetPassword = async () => {
    if (!email) {
      setError("يرجى كتابة البريد الإلكتروني في الخانة أولاً، ثم اضغط على 'نسيت كلمة المرور؟' لإرسال الرابط.");
      setSuccessMessage("");
      return;
    }
    setIsLoading(true);
      ...
      <Card className="w-full max-w-md border rounded-sm shadow-none bg-white overflow-hidden relative">
        ...
        <CardContent className="space-y-6">
          ...
          <p className="text-center text-xs text-slate-500 font-bold">
            ...
            <button onClick={() => { setIsRegister(!isRegister); setError(""); setSuccessMessage(""); setVerificationSent(false); }} className="text-primary font-black mr-2 hover:underline">
